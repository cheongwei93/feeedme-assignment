...
describe("BotManager", () => {
  test("Bot process an order and moves it to COMPLETE after 10 seconds", () => {
    ...
    const botManager = new BotManager(orderManager, logger);
    ...
    expect(orderManager.completed[0].id).toBe(1);
  });
  ...
  test("Removing a bot returns its in-progress order back to pending", () => {
    ...
    expect(orderManager.pending[0].id).toBe(1);
  });
